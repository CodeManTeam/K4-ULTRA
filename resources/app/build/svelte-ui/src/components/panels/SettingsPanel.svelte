<script lang="ts">
  ...
  // ─── Presets ───
  const PRESETS = [
    { name: '🍊 Default', primary: '#ff7829', secondary: '#ff9e47', bg: '#050508', text: '#ffffff', dark: true, dataTheme: 'theme/orange' },
    { name: '🔵 Sky Blue', primary: '#297eff', secondary: '#4790ff', bg: '#0a0a18', text: '#e0e8ff', dark: true, dataTheme: 'theme/blue' },
    { name: '🟡 Sun Yellow', primary: '#ffdb29', secondary: '#ffe047', bg: '#0f0a00', text: '#2a2000', dark: true, dataTheme: 'theme/yellow' },
    { name: '🟣 Violet', primary: '#9429ff', secondary: '#a347ff', bg: '#0a001a', text: '#e8d0ff', dark: true, dataTheme: 'theme/violet' },
    { name: '🟢 Emerald', primary: '#1eb86c', secondary: '#28c777', bg: '#020f08', text: '#d0ffe8', dark: true, dataTheme: 'theme/green' },
    { name: '🌸 Pink', primary: '#ff2970', secondary: '#ff4785', bg: '#0f0008', text: '#ffd0e0', dark: true, dataTheme: 'theme/pink' },
    { name: 'Ultra Purple', primary: '#583cdc', secondary: '#00b4ff', bg: '#050508', text: '#ffffff', dark: true, dataTheme: 'theme/ultra' },
    { name: 'Cyber Green', primary: '#00ff88', secondary: '#00ccff', bg: '#051208', text: '#d0ffe0', dark: true, dataTheme: 'theme/ultra' },
    { name: 'Minimal White', primary: '#333333', secondary: '#888888', bg: '#f0f0f5', text: '#1a1a1a', dark: false, dataTheme: '' },
    { name: 'Paper', primary: '#2c5282', secondary: '#718096', bg: '#fefefe', text: '#1a202c', dark: false, dataTheme: '' },
  ];
  ...
  let zones = $state({
    ...
    sidebar: { ...defaultZone(), ...(saved.zones?.sidebar || {}) },
    stage: { ...defaultZone(), ...(saved.zones?.stage || {}) },
    blockly_bg: { ...defaultZone(), ...(saved.zones?.blockly_bg || {}) },
  });

  const ZONE_LABELS: Record<string,string> = {
    header: 'Header / Top Bar', workspace: 'Workspace / Content',
    sidebar: 'Sidebar / Toolbox', stage: 'Stage / Preview',
    blockly_bg: 'Blockly Canvas'
  };

  function currentZone() { return (zones as any)[activeZoneTab]; }
  ...
  function refreshExtensions() {
    try {
      const K4 = (window as any).__k4; const exts = (K4 && K4._loadedExtensions) || [];
      ...
      loadedExts = exts.map((e: any) => ({ id: e.id, name: e.name || e.id, version: e.version || '?', description: e.description || '', enabled: disabled.indexOf(e.id) === -1 }));
      extError = null;
  ...
  function toggleExtension(id: string) {
    ...
    if (idx === -1) { disabledList = [...disabledList, id]; }
    else { disabledList = [...disabledList.slice(0, idx), ...disabledList.slice(idx + 1)]; }
    loadedExts = loadedExts.map(e => e.id === id ? { ...e, enabled: disabledList.indexOf(id) === -1 } : e);
    try {
      const EXT_DIR = (window as any).__app_dir ? (window as any).__app_dir.replaceAll('\\', '/') + '/extensions' : '';
      if (FS) FS.writeFile(EXT_DIR + '/.disabled.json', JSON.stringify(disabledList, null, 2));
    } catch(_) {}
  }
  ...
  function saveSettings() {
    ...
    if (general.theme !== oldTheme) {
      uiStore.notify({ type: 'warning', message: 'Loading theme changed. Reload now?', duration: 5000 });
      setTimeout(() => { if (confirm('Reload to apply loading theme?')) location.reload(); }, 400);
    } else {
      uiStore.notify({ type: 'success', message: 'Settings saved!' });
    }
    onclose();
  }
</script>
...
<div class="k4-settings-panel" onclick={(e) => e.stopPropagation()}>
  ...
  <div class="k4-panel-body">
    ...
    <section class="k4-section">
      ...
      <div class="k4-setting-row"><label class="k4-setting-label">Primary</label><div class="k4-color-row"><input type="color" class="k4-color-pick" bind:value={appearance.primaryColor} /><code class="k4-color-hex">{appearance.primaryColor}</code></div></div>
      ...
      {#if appearance.glassMorphism}
        <div class="k4-setting-row"><label class="k4-setting-label">Blur</label><div class="k4-stepper"><button class="k4-step-btn" onclick={() => appearance.glassBlur = Math.max(4, appearance.glassBlur - 2)}>-</button><span class="k4-step-value">{appearance.glassBlur}px</span><button class="k4-step-btn" onclick={() => appearance.glassBlur = Math.min(40, appearance.glassBlur + 2)}>+</button></div></div>
        <div class="k4-setting-row"><label class="k4-setting-label">Opacity</label><input type="range" class="k4-slider" min="0" max="50" step="1" bind:value={appearance.glassOpacity} /><code class="k4-color-hex">{appearance.glassOpacity}%</code></div>
      {/if}
    </section>
    ...
    <!-- ═══ ZONE CUSTOMIZATION ═══ -->
    <section class="k4-section">
      <h3 class="k4-section-title">🖼️ Zone Customization</h3>
      <p style="font-size:11px;color:var(--oc-white-40);margin:0 0 10px 0;">Customize each UI area independently: background, image, blur, opacity, etc.</p>

      <!-- Zone tabs -->
      <div class="k4-zone-tabs">
        {#each Object.keys(ZONE_LABELS) as zid}
          <button class="k4-zone-tab" class:k4-zone-tab-active={activeZoneTab === zid} onclick={() => activeZoneTab = zid}>
            {ZONE_LABELS[zid]}
          </button>
        {/each}
      </div>
    ...
    <!-- ═══ EDITOR ═══ -->
    <section class="k4-section">
      <h3 class="k4-section-title">✏️ Editor</h3>
      <div class="k4-setting-row"><label class="k4-setting-label">Font Size</label><div class="k4-stepper"><button class="k4-step-btn" onclick={() => editor.fontSize = Math.max(10, editor.fontSize - 1)}>-</button><span class="k4-step-value">{editor.fontSize}px</span><button class="k4-step-btn" onclick={() => editor.fontSize = Math.min(24, editor.fontSize + 1)}>+</button></div></div>
      <Checkbox label="Snap to Grid" bind:checked={editor.snapToGrid} />
      <Checkbox label="Category Icons" bind:checked={editor.showCategoryIcons} />
      <Checkbox label="Compact Mode" bind:checked={editor.compactMode} />
      <Checkbox label="Line Numbers" bind:checked={editor.lineNumbers} />
      <Checkbox label="Auto Complete" bind:checked={editor.autoComplete} />
      <div style="margin-top:8px;padding:6px 10px;background:var(--oc-gray-850);border-radius:4px;font-size:10px;color:var(--oc-white-40);">ℹ️ Editor settings persist locally, applied on next launch.</div>
    </section>
    ...
    <section class="k4-section">
      ...
      <div class="k4-ext-list">
        {#if loadedExts.length === 0}
          ...
          <div class="k4-ext-hint">Place .k4ultra files in the extensions folder</div>
        {:else}
          {#each loadedExts as ext}
            <div class="k4-ext-item" class:k4-ext-disabled={!ext.enabled}>
              <div class="k4-ext-info"><div class="k4-ext-name">{ext.name}</div><div class="k4-ext-meta"><span class="k4-ext-version">{ext.version}</span>{#if ext.description}<span class="k4-ext-desc">{ext.description}</span>{/if}</div></div>
              <label class="k4-ext-toggle"><input type="checkbox" checked={ext.enabled} onchange={() => toggleExtension(ext.id)} /><span class="k4-ext-toggle-knob"></span></label>
            </div>
          {/each}
        {/if}
      </div>
    ...
    <!-- ═══ ABOUT ═══ -->
    <section class="k4-section">
      <h3 class="k4-section-title">ℹ️ About</h3>
      <div class="k4-about-box">
        ...
        <div class="k4-about-line"><span class="k4-about-label">Developer</span><span class="k4-about-val">{aboutInfo.author}</span></div>
        <div class="k4-about-line"><span class="k4-about-label">QQ</span><span class="k4-about-val">{aboutInfo.qq}</span></div>
        <div class="k4-about-line"><span class="k4-about-label">Group</span><span class="k4-about-val">{aboutInfo.group}</span></div>
      </div>
    </section>
  </div>
...
<style>
  ...
  .k4-setting-row{display:flex;align-items:center;justify-content:space-between;min-height:36px;margin:4px 0}
  ...
  /* Zone tabs */
  .k4-zone-tabs{display:flex;gap:4px;margin-bottom:12px;flex-wrap:wrap}
  .k4-zone-tab{padding:6px 12px;border:1px solid var(--oc-gray-600);border-radius:6px;background:var(--oc-gray-850);color:var(--oc-white-60);font-size:12px;cursor:pointer;transition:all 0.15s;font-family:var(--oc-font-inter)}
  .k4-zone-tab:hover{background:var(--oc-gray-700);color:var(--oc-white-80)}
  .k4-zone-tab-active{background:var(--oc-primary-700)!important;color:#fff!important;border-color:var(--oc-primary-600)!important}
  .k4-zone-panel{background:var(--oc-gray-850);border-radius:10px;padding:14px;margin-top:8px}
