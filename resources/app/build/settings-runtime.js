/**
 * K4 Ultra Settings Runtime Bridge v3
 * Full zone customization engine: header, workspace, sidebar, stage + per-zone glass/blur/bg.
 */
(function () {
  'use strict';
  ...
  function generateFullPaletteCSS(primaryHex, isDark) {
    ...
    var css = '';
    var shadeDefs = [
      { key: '--theme-color-0',dl:20},{ key: '--theme-color-1',dl:16},{ key: '--theme-color-2',dl:12},
      { key: '--theme-color-3',dl:8}, { key: '--theme-color-4',dl:4}, { key: '--theme-color-5',dl:0},
      { key: '--theme-color-6',dl:-4},{ key: '--theme-color-7',dl:-8},{ key: '--theme-color-8',dl:-12},
      { key: '--theme-color-9',dl:-16},{ key: '--theme-color-10',dl:-20},{ key: '--theme-color-n1',dl:-30},{ key: '--theme-color-n2',dl:-45}
    ];
    for (var i=0;i<shadeDefs.length;i++){var sd=shadeDefs[i];css+=sd.key+':'+hslToRgbString(h,s,Math.max(0,Math.min(100,l+sd.dl)))+';';}
    ...
    if(isDark){css+='--theme-color-t:255,255,255;--theme-color-t1:200,204,210;--theme-color-t3:155,160,170;--theme-color-t5:110,115,125;--theme-color-t7:73,78,92;';}
    else{css+='--theme-color-t:30,30,35;--theme-color-t1:60,60,70;--theme-color-t3:90,90,100;--theme-color-t5:120,120,130;--theme-color-t7:150,150,160;';}
    if(isDark){css+='--theme-color-grey-0:255,255,255;--theme-color-grey-10:235,235,240;--theme-color-grey-20:210,210,218;--theme-color-grey-30:185,185,192;--theme-color-grey-40:160,160,168;--theme-color-grey-50:135,135,143;--theme-color-grey-60:110,110,118;--theme-color-grey-70:85,85,93;--theme-color-grey-80:60,60,68;--theme-color-grey-90:40,40,48;--theme-color-grey-100:25,25,33;';}
    else{css+='--theme-color-grey-0:25,25,33;--theme-color-grey-10:55,55,65;--theme-color-grey-20:85,85,95;--theme-color-grey-30:115,115,125;--theme-color-grey-40:145,145,155;--theme-color-grey-50:175,175,182;--theme-color-grey-60:195,195,200;--theme-color-grey-70:215,215,218;--theme-color-grey-80:235,235,238;--theme-color-grey-90:245,245,247;--theme-color-grey-100:255,255,255;';}
  ...
  function applyPaletteToStyle(css) {
    var pairs=css.split(';'),root=document.documentElement.style;
    for(var i=0;i<pairs.length;i++){var p=pairs[i].trim();if(!p)continue;var colon=p.indexOf(':');if(colon<1)continue;root.setProperty(p.substring(0,colon).trim(),p.substring(colon+1).trim());}
  }
  function clearInlinePalette() {
    var root=document.documentElement.style,themeProps=[];
    for(var i=0;i<root.length;i++){if(root[i].indexOf('--theme-color')===0)themeProps.push(root[i]);}
    for(var j=0;j<themeProps.length;j++){root.removeProperty(themeProps[j]);}
    var pel=document.getElementById('k4-theme-palette');if(pel)pel.textContent='';
  }

  function applyThemeToBody(theme) { if(!theme||!document.body)return;document.body.setAttribute('data-theme','theme/'+theme); }
  function setBodyDataTheme(dt) { if(!dt||!document.body)return;document.body.setAttribute('data-theme',dt); }

  function readNativeSettings() { try{return JSON.parse(localStorage.getItem('editor_settings')||'{}');}catch(_){return{};} }
  function writeNativeSettings(u) { try{var n=readNativeSettings();for(var k in u){if(u.hasOwnProperty(k))n[k]=u[k];}localStorage.setItem('editor_settings',JSON.stringify(n));}catch(_){} }

  // ═══════════════════════════════════════════════════
  //  ZONE CUSTOMIZATION ENGINE
  // ═══════════════════════════════════════════════════

  var zoneStyles = {};  // { zoneId: cssString }
  var zoneStyleEl = null;
  var zoneObserver = null;
  var zoneConfig = {};  // current zone settings from k4-settings.json

  // Zone definitions: name, DOM selectors to try (in priority order)
  var ZONE_DEFS = {
    header: {
      label: 'Header / Top Bar',
      selectors: [
        '[id*="header"]:not([id*="setting"])',
        '[class*="header_wrap"]',
        '[class*="Header_wrap"]',
        '[class*="menubar"]',
        '[class*="menu-bar"]',
      ...
      findParent: true, // if selector matches a child, style its parent block
    },
    workspace: {
      label: 'Workspace / Content',
      selectors: [
        '[id*="content_draggable"]',
        '[class*="content-wrap"]',
        '[class*="workspace"]',
        '.blocklyWorkspace',
        '.blocklySvg',
  ...
  function buildZoneCSS(zoneId, cfg) {
    if (!cfg || !cfg.enabled) return '';
    var css = '';
    var selector = '[data-k4-zone="'+zoneId+'"]';
    var props = [];

    if (cfg.bgColor) props.push('background-color:'+cfg.bgColor+'!important');
    if (cfg.bgImage) {
      props.push('background-image:url('+cfg.bgImage+')!important');
      props.push('background-size:'+(cfg.bgSize||'cover')+'!important');
      props.push('background-position:'+(cfg.bgPosition||'center')+'!important');
      props.push('background-repeat:no-repeat!important');
    }
    if (cfg.opacity !== undefined && cfg.opacity < 100) props.push('opacity:'+(cfg.opacity/100)+'!important');
    if (cfg.blur && cfg.blur > 0) {
      props.push('backdrop-filter:blur('+cfg.blur+'px)!important');
  ...
  // Find and mark DOM elements for each zone
  function tagZoneElements(zoneId, def) {
    var found = false;
      ...
      try {
        ...
        if (els && els.length > 0) {
          for (var e = 0; e < els.length; e++) {
            var el = els[e];
            if (el.hasAttribute('data-k4-zone')) continue; // already tagged
            if (def.findParent && el.parentElement) {
              el = el.parentElement;
              if (el.hasAttribute('data-k4-zone')) continue;
            }
            el.setAttribute('data-k4-zone', zoneId);
            found = true;
          }
          if (found) {
            console.log('[K4 Zone] Tagged zone "'+zoneId+'" via selector: '+selectors[s]);
            break;
          }
        }
      } catch(e) {}
  ...
  // Scan DOM and tag all zones
  function scanAllZones() {
    var found = 0;
    for (var zid in ZONE_DEFS) {
      if (ZONE_DEFS.hasOwnProperty(zid)) {
        if (tagZoneElements(zid, ZONE_DEFS[zid])) found++;
      }
    }
    return found;
  }

  // Apply all zone styles
  function applyZoneStyles(cfg) {
    zoneConfig = cfg || {};
    var allCSS = '';
    for (var zid in ZONE_DEFS) {
      if (ZONE_DEFS.hasOwnProperty(zid)) {
        var zcfg = zoneConfig[zid] || {};
        var css = buildZoneCSS(zid, zcfg);
        zoneStyles[zid] = css;
        allCSS += css;
      }
    }
    if (!zoneStyleEl) {
      zoneStyleEl = ensureStyleEl('k4-zone-styles');
    }
    zoneStyleEl.textContent = allCSS;

    // Also tag elements
    var tagged = scanAllZones();
    if (tagged === 0) {
      // Retry after React renders
      setTimeout(function() {
        var t = scanAllZones();
        if (t > 0) applyZoneStyles(zoneConfig); // re-apply after tagging
      }, 3000);
    }
  }

  // Start zone observer for dynamically added elements
  function startZoneObserver() {
    if (zoneObserver) return;
    zoneObserver = new MutationObserver(function() {
      scanAllZones();
      // Re-apply styles for any newly tagged elements
      if (zoneStyleEl && zoneConfig) {
        var allCSS = '';
        for (var zid in zoneStyles) {
          if (zoneStyles.hasOwnProperty(zid)) allCSS += zoneStyles[zid];
        }
        zoneStyleEl.textContent = allCSS;
      }
    });
    if (document.body) {
      ...
      setTimeout(startZoneObserver, 500);
    }
  }

  // ═══ Appearance (unchanged core) ═══
  function applyAppearance(a) {
    a=a||{};
    var p=a.primaryColor||'#583cdc',s=a.secondaryColor||'#00b4ff',bg=a.bgColor||'#050508';
    var root=document.documentElement.style;
    root.setProperty('--k4-primary',p);root.setProperty('--k4-primary-rgb',hexToRgbString(p));
    ...
    if(!isNative&&a.primaryColor){
      ...
      applyPaletteToStyle(generateFullPaletteCSS(p,brightness<=128));
    }else if(isNative){clearInlinePalette();}
    ...
    if(a.bgImage){
      ...
      if(!ov){ov=document.createElement('div');ov.id='k4-bg-image-overlay';ov.style.cssText='position:fixed;inset:0;z-index:-1;pointer-events:none;';document.body.appendChild(ov);}
      ov.style.backgroundImage='url('+a.bgImage+')';ov.style.backgroundSize='cover';ov.style.backgroundPosition='center';
    }
    ...
    if(document.body){document.body.classList.remove('k4-dark','k4-light');document.body.classList.add(brightness2>128?'k4-light':'k4-dark');}
    if(a.dataTheme){setBodyDataTheme(a.dataTheme);}else if(!isNative&&a.primaryColor){setBodyDataTheme('theme/ultra');}
  ...
  function applyDebug(d){d=d||{};window.__k4_debug=window.__k4_debug||{};if(d.verboseLogging!==undefined)window.__k4_debug.verbose=d.verboseLogging;if(d.perfMonitor!==undefined)window.__k4_debug.perf=d.perfMonitor;if(d.showBlockIds!==undefined)writeNativeSettings({showBlockIds:d.showBlockIds});}
  ...
  function integrateNativeHeader(){
    var ha=0;
    function hk(){var btn=document.getElementById('header-setting-btn');if(btn&&!btn.__k4_hooked){btn.__k4_hooked=true;btn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(window.__k4bus)window.__k4bus.emit('open-settings');});}if(!document.getElementById('header-setting-btn')&&ha<100){ha++;setTimeout(hk,300);}}
    setTimeout(hk,2000);
  }
  ...
  function applyBlocklyTheme(){
    try{
      var bw=window.Blockly;if(!bw||!bw.mainWorkspace)return;
      var ws=bw.mainWorkspace,grid=ws.getGrid?ws.getGrid():null;if(!grid)return;
  ...
  console.log('[K4 Settings] Runtime bridge v3 (zones engine) initialized');
})();
